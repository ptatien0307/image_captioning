...
class Decoder(torch.nn.Module):
    def __init__(self, vocab_size, embed_dim, attention_dim, encoder_dim, decoder_dim, drop_prob=0.3):
        ...
        self.embedding = torch.nn.Embedding(vocab_size, embed_dim).to(device)# Embedding layer
        self.lstm = torch.nn.LSTMCell(input_size=embed_dim + encoder_dim, hidden_size=decoder_dim, bias=True).to(device) # LSTM layer
        self.fcn = torch.nn.Linear(decoder_dim, self.vocab_size).to(device)# Linear layer
